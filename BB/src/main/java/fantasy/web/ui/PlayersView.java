...
public class PlayersView extends CustomComponent implements ContentUpdateable{
	...
	@AutoGenerated
	private CheckBox justOwnPlayersCB;
	@AutoGenerated
	...
	public PlayersView() {
		...
		justOwnPlayersCB.addListener(new Property.ValueChangeListener() {
	...
	/**
	 * Calculate average of column values. Caller have to make sure parametercolumn is numerable.
	 * @param propertyId - column property
	 * @return
	 */
	private String getColumnAverage(Object propertyId){
		...
		//show just first two digits 
		DecimalFormat twoDForm = new DecimalFormat("#.##");
		return twoDForm.format(sum / rows);
	}
	
	

	public void updateContent(){
		//show checkbox if user role is manager (manager has team)
		justOwnPlayersCB.setVisible(FantasyApplication.getInstance().getCurrentUser().getUserRole() == Role.MANAGER);
	...
	/**
	 * if checkbox is selected add only own team members to player container
	 */
	private void refreshPlayerContainer(){
		UserClass currentUser = FantasyApplication.getInstance().getCurrentUser();
		
		if(currentUser.getUserRole() == Role.MANAGER && justOwnPlayersCB.booleanValue()){
	...
	private VerticalLayout buildMainLayout() {
		...
		mainLayout.addComponent(justOwnPlayersCB);
